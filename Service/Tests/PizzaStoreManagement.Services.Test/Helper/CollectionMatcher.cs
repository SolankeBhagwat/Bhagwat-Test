namespace PizzaStoreManagement.Services.Test.Helper
{
    ...
    public static class CollectionMatcher<T> where T : class
    {
        public static bool Match(IEnumerable<T> actualItems, IEnumerable<T> expectedItems)
        {
            ...
            foreach (var actualItem in actualItems)
            {
                ...
                {
                    if (PropertyMatcher<T>.Match(actualItem, expectedItem))
                    {
                        IsMatch = true;
    ...
    public static class PropertyMatcher<T> where T : class
    {
        public static bool Match(T actual, T expected)
        {
            ...
            foreach (var property in typeof(T).GetProperties().Where(p => p.GetIndexParameters().Length == 0))
            {
                ...
                else if (!(property.PropertyType == typeof(Collection<string>) || property.PropertyType==typeof(List<Topping>)))
                {
